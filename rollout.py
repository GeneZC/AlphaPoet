...
import numpy as np
from calculate_bleu import calculate_bleu
...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        ...
        #####################################################################################################
        self.weight = tf.identity(self.lstm.weight)
        self.mlp_b = tf.identity(self.lstm.mlp_b)
    ...
    def get_reward(self, sess, generated_x, hiddens, rollout_num, discriminator):
        rewards = []
        for i in range(rollout_num):
            for given_num in range(1, self.sequence_length):
                feed = {self.x: generated_x, self.hiddens:hiddens, self.given_num: given_num}
            ...
            feed = {discriminator.input_x: generated_x, discriminator.dropout_keep_prob: 1.0}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
                rewards.append(ypred)
