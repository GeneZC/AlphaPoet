...
class MobileNet:
    ...
    def __init__(self, batch_size):
        ...
        self.l2_strength = 4e-5
        self.bias = 0.0
        self.learning_rate = 1e-3
        self.dropout_keep_prob = 0.99
        self.pretrained_path = "pretrained_weights/mobilenet_v1.pkl"
        self.X = None
        self.y = None
        self.logits = None
        ...
        self.pretrained_path = os.path.realpath(self.pretrained_path)
    ...
    def __init_network(self):
        with tf.variable_scope('mobilenet_encoder'):
            ...
            # Model is here!
            conv1_1 = conv2d('conv_1', preprocessed_input, num_filters=int(round(32 * self.width_multiplier)),
                             kernel_size=(3, 3),
                             padding='SAME', stride=(2, 2), activation=tf.nn.relu6,
                             batchnorm_enabled=self.batchnorm_enabled,
                             is_training=self.is_training, l2_strength=self.l2_strength, bias=self.bias)
            ...
            dropped = dropout(avg_pool, self.dropout_keep_prob, self.is_training)
            self.logits = flatten(conv2d('fc', dropped, kernel_size=(1, 1), num_filters=self.num_classes,
                                         l2_strength=self.l2_strength,
                                         bias=self.bias))
    ...
    def __init_output(self):
        with tf.variable_scope('output'):
            ...
            self.y_output = tf.nn.softmax(self.logits)
            self.y_out_argmax = tf.argmax(self.y_output, axis=-1, output_type=tf.int32)
